...
class _LocationSelectionScreenState extends State<LocationSelectionScreen> {
  ...
  try {
    // URL encode the query and add proper headers
    final encodedQuery = Uri.encodeComponent(query);
    final response = await http.get(
      Uri.parse("$nominatimApiUrl?q=$encodedQuery&format=json&limit=10"),
      headers: {
    ...
    if (response.statusCode == 200) {
      print('Search response: ${response.body}'); // Debug - check the response
      ...
      for (var item in data) {
        try {
          ...
          print('Error parsing result item: $e');
        }
      }
...
Future<void> _selectSearchResult(Map<String, dynamic> result) async {
  ...
  // Debug print to verify the selected location values
  print('Selected Location - Latitude: $latitude, Longitude: $longitude, Address: $address');
  ...
  if (userId != null) {
    try {
      await _firestore.collection('user_locations').doc(userId).set({
        'latitude': latitude,
        'longitude': longitude,
        'address': address, // Optionally store the address
        'updatedAt': FieldValue.serverTimestamp(),
      }, SetOptions(merge: true));

      // Debug print to verify Firestore update
      print('Firestore updated with - Latitude: $latitude, Longitude: $longitude');
    ...
    print('No user ID available to update Firestore');
    // Still call the callback with the selected values
    widget.onLocationSelected(latitude, longitude, address);
    Navigator.pop(context);
  }
}
                      ...
                      markers: [
                        ...
                        Marker(
                          ...
                          builder: (ctx) => const Icon(
                            ...
                            color: Color.fromARGB(255, 16, 0, 161),
                            size: 40,
                          ),
                        ),
